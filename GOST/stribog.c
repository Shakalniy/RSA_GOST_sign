...
//#include <stdlib.h>
//#include <immintrin.h>
//#include <malloc.h> // Для _mm_malloc
//
//#pragma GCC optimize("O3,unroll-loops,inline-functions")
//#pragma GCC target("ssse3,pclmul,avx2")
//
...
//}
//
//// Преобразование E (минимизация копирования)
//void e_transform(uint8_t* state, uint8_t* K, size_t block_size) {
//    uint8_t temp[BLOCK_SIZE];
//    uint8_t* current = state;
//    uint8_t* next = temp;
//    for (int i = 0; i < 12; i++) {
//        for (size_t j = 0; j < block_size; j++) next[j] = current[j] ^ K[j];
//        for (size_t j = 0; j < block_size; j++) next[j] = S_BOX[next[j]];
//        for (size_t j = 0; j < block_size; j++) next[j] = next[P_TABLE[j]];
//        l_transform(next, block_size);
...
//        l_transform(K, block_size);
//    }
//    if (current != state) memcpy(state, current, block_size);
//}
//
//// Функция g_N (минимизация копирования)
//void g_N(uint8_t* h, const uint8_t* m, const uint8_t* N, uint8_t* out, size_t block_size) {
//    uint8_t K[BLOCK_SIZE];
//    for (size_t i = 0; i < block_size; i++) K[i] = h[i] ^ N[i];
//    for (size_t i = 0; i < block_size; i++) K[i] = S_BOX[K[i]];
//    for (size_t i = 0; i < block_size; i++) K[i] = K[P_TABLE[i]];
//    l_transform(K, block_size);
//    memcpy(out, m, block_size);
//    e_transform(out, K, block_size);
//    for (size_t i = 0; i < block_size; i++) out[i] ^= h[i] ^ m[i];
//}
//
//// Сложение по модулю 2^512 (in-place, два аргумента)
//static inline void add_mod512(uint8_t* a, const uint8_t* b) {
...
//}
//
//// Финальное преобразование (минимизация копирования)
//void final_transformation(uint8_t* h, const uint8_t* Sigma, const uint8_t* N,
//                          uint8_t* out, int bit_size) {
//    uint8_t zero[BLOCK_SIZE] = {0};
//    g_N(h, N, zero, h, BLOCK_SIZE); // Перезаписываем h
//    g_N(h, Sigma, zero, out, BLOCK_SIZE);
//    if (bit_size == 256) memcpy(out, out + 32, 32);
//}
//
//// Основная функция хеширования согласно ГОСТ 34.11—2018
//void stribog_hash(const uint8_t* msg, size_t len, uint8_t* out, int bit_size) {
...
//    uint8_t* h = _mm_malloc(BLOCK_SIZE, 32);
//    uint8_t* N = _mm_malloc(BLOCK_SIZE, 32);
//    uint8_t* Sigma = _mm_malloc(BLOCK_SIZE, 32);
//    uint8_t* m_last = _mm_malloc(BLOCK_SIZE, 32);
//    uint8_t* tmp = _mm_malloc(BLOCK_SIZE, 32);
//
//    memcpy(h, (bit_size == 512) ? IV_512 : IV_256, BLOCK_SIZE);
//    memset(N, 0, BLOCK_SIZE);
//    memset(Sigma, 0, BLOCK_SIZE);
//
//    // Подготовка
//    size_t full_blocks = len / BLOCK_SIZE;
//    size_t rem = len % BLOCK_SIZE;
